...
private fun AssistantLocalToolContent(
    ...
    Column(
        ...
        CardGroup {
            ...
            item(
                headlineContent = {
                    Text(stringResource(R.string.assistant_page_local_tools_usage_stats_title))
                },
                supportingContent = {
                    Text(stringResource(R.string.assistant_page_local_tools_usage_stats_desc))
                },
                trailingContent = {
                    Switch(
                        checked = assistant.localTools.contains(LocalToolOption.UsageStats),
                        onCheckedChange = { toggleLocalTool(LocalToolOption.UsageStats, it) }
                    )
                }
            )
            item(
